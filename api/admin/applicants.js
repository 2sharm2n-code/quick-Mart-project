...
export default async function (req, res) {
  if (req.method === "GET") {
    const { rows } = await db.query("SELECT a.id, a.application_ref, a.full_name, a.national_id, a.phone, a.email, a.applied_position, a.home_county, a.preferred_branch_county, a.status, a.payment_status, a.total_amount_kes, COALESCE((SELECT p.amount_kes FROM payments p WHERE p.applicant_id = a.id AND p.status = 'completed' ORDER BY p.completed_at DESC NULLS LAST, p.requested_at DESC LIMIT 1), 0) AS paid_amount_kes, (SELECT p.mpesa_receipt_number FROM payments p WHERE p.applicant_id = a.id AND p.status = 'completed' ORDER BY p.completed_at DESC NULLS LAST, p.requested_at DESC LIMIT 1) AS mpesa_receipt_number, a.applied_at, a.appointment_date, a.appointment_time, a.appointment_instructions FROM applicants a ORDER BY a.applied_at DESC LIMIT 500");
