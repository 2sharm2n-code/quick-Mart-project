...
app.post('/api/apply', async (req, res) => {
  ...
  try {
    ...
    const result = await client.query(
      `INSERT INTO applications (job_id, candidate_id, lookup_code) VALUES ($1,$2,$3) RETURNING *`,
...
app.get('/api/lookup/:code', async (req, res) => {
  try {
    const result = await pool.query(`
      ...
      FROM applications a JOIN jobs j ON a.job_id=j.id JOIN candidates c ON a.candidate_id=c.id
      WHERE a.lookup_code=$1`, [String(req.params.code || '').trim()]);
...
app.patch('/api/applications/:id/status', async (req, res) => {
  ...
  try {
    ...
    await client.query('INSERT INTO stage_history (application_id, stage_name, status) VALUES ($1,$2,$3)', [req.params.id, stage, 'Pending']);
    await client.query('COMMIT');
    res.json({ ...result.rows[0], status: req.body.status });
  } catch (err) { await client.query('ROLLBACK'); console.error(err); res.status(500).json({ error: 'Server error' }); }
  finally { client.release(); }
});
...
app.post('/api/interviews', async (req, res) => {
  ...
  try {
    const result = await pool.query(`INSERT INTO interviews (application_id, interviewer_name, interview_type, scheduled_time, duration_minutes, meeting_link)
      VALUES ($1,$2,$3,$4,$5,$6) RETURNING *`, [application_id, interviewer_name || null, interview_type || 'In-person', scheduled_time, duration_minutes || 60, meeting_link || null]);
  ...
  } catch (err) { console.error(err); res.status(500).json({ error: 'Failed to schedule interview' }); }
});

app.get('/api/applications/:id/interviews', async (req, res) => {
  try { const result = await pool.query('SELECT * FROM interviews WHERE application_id=$1 ORDER BY scheduled_time DESC', [req.params.id]); res.json(result.rows); }
  catch (err) { console.error(err); res.status(500).json({ error: 'Server error' }); }
});

app.get('/api/applications/:id/stages', async (req, res) => {
  try { const result = await pool.query('SELECT * FROM stage_history WHERE application_id=$1 ORDER BY created_at DESC', [req.params.id]); res.json(result.rows); }
  catch (err) { console.error(err); res.status(500).json({ error: 'Server error' }); }
});
...
app.post('/api/pay/mpesa', async (req, res) => {
  ...
  if (!application_id || !phone || !amount || !MPESA_PASSKEY || !MPESA_CALLBACK_URL) return res.status(400).json({ error: 'application_id, phone, amount and M-PESA environment configuration are required' });
  try {
    ...
    const response = await axios.post('https://sandbox.safaricom.co.ke/mpesa/stkpush/v1/processrequest', {
      ...
      Amount: Math.round(Number(amount)), PartyA: phone, PartyB: MPESA_SHORTCODE, PhoneNumber: phone, CallBackURL: MPESA_CALLBACK_URL,
      AccountReference: `App-${application_id}`, TransactionDesc: 'Recruitment Fee'
  ...
  } catch (error) { console.error(error.response ? error.response.data : error.message); res.status(500).json({ error: 'M-PESA initiation failed' }); }
});
...
app.post('/api/mpesa/callback', async (req, res) => {
  try {
    ...
    await pool.query(`UPDATE payments SET payment_status=$1, mpesa_receipt_number=COALESCE($2,mpesa_receipt_number),
      ...
      WHERE metadata->>'CheckoutRequestID'=$6 OR metadata->>'checkoutRequestID'=$6`, [status, receipt, phone, amount, req.body, checkoutRequestID]);
    res.json({ result: 'OK' });
...
app.post('/api/send-email', async (req, res) => {
  ...
  try {
    ...
    await pool.query(`INSERT INTO communications (application_id, channel, direction, subject, body, recipient) VALUES ($1,'Email','Outbound',$2,$3,$4)`, [application_id, subject, message, candidate_email]);
...
const initDB = async () => {
  ...
  try { await pool.query(fs.readFileSync(schemaPath, 'utf8')); console.log('🗄️ Database tables initialized.'); }
